            ...
            axios.post("http://localhost:3001/login", userData)
                .then(response => {
                    if (response.status === 200) {
                        ...
                        let jwtpayLoad = response.data.payload
                        localStorage.setItem('id', JSON.stringify(jwtpayLoad))
                                ...
                                <form onSubmit={onSubmit}>
                                    ...
                                    <div className="py-2">
                                        ...
                                        <div className="flex-item">
                                            <input type={showPassword ? 'text' : 'password'} id="psw" className="form-control " placeholder="Minimum 8 charecter" onChange={(e) => setPassword(e.target.value)} value={password} />
                                            {showPassword ? (
                            ...
                            <div className="side-item">
                                ...
                                <div className="item2 mt-2">
                                    <div className="my-1">
                                        ...
                                        <span className="profile-item1 px-1"> One click apply using pabjobs profile.</span>
                                    </div>
                                    <div className="my-1">
                                        <i className="fa-solid fa-check icon-item1"></i>
                                        <span className="profile-item1 px-1"> Get relavant jobs recommedations.</span>
                                    </div>
                                    <div className="my-1">
                                        <i className="fa-solid fa-check icon-item1"></i>
                                        <span className="profile-item1 px-1"> Showcase profile to top companies and consultant.</span>
                                    </div>
                                    <div className="my-1">
                                        <i className="fa-solid fa-check icon-item1"></i>
                                        <span className="profile-item1 px-1"> know application status on applied jobs.</span>
                                    </div>
                                </div>
                                <button className="free-item my-2 mb-4 " id="registerBtn" onClick={GotoAuth} > Register for Free</button>
                            </div>
                        </div>
                    </div>
                </div>
